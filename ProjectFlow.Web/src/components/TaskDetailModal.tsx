...
import { Task, taskApi } from "@/lib/api/tasks";
...
export function TaskDetailModal({ taskId, isOpen, onClose, onUpdate }: TaskDetailModalProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [taskData, subtasksData, commentsData] = await Promise.all([
        taskApi.getById ? taskApi.getById(taskId) : Promise.resolve(null), // Assuming getById exists or fallback
  ...
  const handleAddSubtask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedSubtasks = await subtaskApi.getByTask(taskId);
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-[#1a1a2e] w-full max-w-4xl h-[85vh] flex flex-col relative overflow-hidden rounded-2xl shadow-[0_25px_60px_rgba(0,0,0,0.3)] border border-gray-200 dark:border-white/10 animate-in zoom-in-95 duration-300">
        <button 
          ...
          className="p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors z-20 text-gray-400 hover:text-gray-600 dark:hover:text-white"
        >
          <X size={18} />
        </button>
          ...
          <div className="flex flex-1 overflow-hidden flex-col md:flex-row">
                ...
                <div className="flex flex-wrap gap-8 py-5 border-y border-gray-100 dark:border-white/10">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium capitalize text-gray-800 dark:text-gray-200">{task.status.replace('_', ' ')}</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex-1 flex flex-col bg-gray-50 dark:bg-[#12122a] border-l border-gray-200 dark:border-white/10">
              <div className="p-5 border-b border-gray-200 dark:border-white/10 flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Activity & Comments</h3>
              </div>
